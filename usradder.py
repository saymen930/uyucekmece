...
def banner():
    ...
    print(f'{info}{g} Kusha Mühendislik{rs}')
...
print(f'{info}{g} Eklemeler Yapılıyor {group_name}{rs}\n')
n = 0
added_users = []
for user in users:
    n += 1
    ...
    if n % 50 == 0:
        print(f'{sleep}{g} Ban Riski Dolayısıyla 2 Dakika Bekleme{rs}')
        time.sleep(120)
    try:
        if user['username'] == "":
            continue
        ...
        usr_id = user['user_id']
        print(f'{attempt}{g} Ekleniyor{usr_id}{rs}')
        print(f'{sleep}{g} 30 Saniye Bekleme{rs}')
        time.sleep(30)
    except PeerFloodError:
        #time.sleep()
        os.system(f'del {file}')
        sys.exit(f'\n{error}{r} Flood Hatası{rs}')
    except UserPrivacyRestrictedError:
        print(f'{error}{r} Bu Kullanıcı Gruplara Eklenmeyi Engellemiş{rs}')
        continue
    except KeyboardInterrupt:
        print(f'{error}{r} Uygulama Durduruldu{rs}')
        update_list(users, added_users)
        if not len(users) == 0:
            print(f'{info}{g} Kalan Kullanıcılar {file}')
            logger = Relog(users, file)
            logger.start()
    except:
        print(f'{error}{r} Ekleme Yapılırken Bir Hata Oluştu{rs}')
        continue
#os.system(f'del {file}')
input(f'{info}{g}Ekleme Tamamlandı, Enter tuşuna basarak çıkabilirsiniz...')
sys.exit()
